...
def similarity(title_a: str, title_b: str) -> float:
    ...
    return len(a.intersection(b)) / max(len(a), len(b), 1)

if __name__ == "__main__":

    console = Console()

    downloaded: List[Set[str]] = []
    for root, dirs, files in os.walk("papers"):
        for file in files:
            filename, _ = os.path.splitext(file)
            downloaded.append(filename.replace("_", " "))
    console.print(f"Downloaded {len(downloaded)} papers.")


    def is_downloaded(title, threshold: float = 0.6):
        return max(map(lambda x: similarity(title, x), downloaded)) >= threshold

    console.print(f"Total {len(papers)} papers.")
    papers = [row for row in papers if not is_downloaded(row.split("\n", 1)[0])]
    console.print(f"Remaining {len(papers)} papers.")
    ...
    for paper in bar:
        ...
        try:
            ...
            for result in results:
                if similarity(result.title, title) >= 0.6:
                    result.download_pdf("papers", f"{result.title.replace(' ', '_')}.pdf")
                    bar.set_description(f"{missing}/{len(papers)} missed")
                    break
            else:
                missing += 1
                max_similarity = max(map(lambda x: similarity(x.title, title), results)) if results else .0
                bar.set_description(f"{missing}/{len(papers)} missed, {max_similarity:.2f}")

        except Exception as e:
            missing += 1
            exception += 1
            bar.set_description(f"{missing}/{len(papers)} missed, {exception} exceptions")
