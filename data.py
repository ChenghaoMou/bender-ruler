...
class ArxivDownloader:
    ...
    @staticmethod
    def clean_title(title: str) -> Set[str]:
        return set(regex.sub(r"[ \p{P}]+", " ", title.lower()).split(" "))

    def similarity(self, title_x: str, title_y: str) -> float:
        x, y = self.clean_title(title_x), self.clean_title(title_y)
        return min(len(x.intersection(y)) / max(len(x), len(y), 1), 1.0)

    def search(self, title: str, max_results: int = 10) -> Optional[arxiv.Result]:
        ans = None
        ...
        try:
            ...
            for result in search.results:
                if self.similarity(result.title, title) >= self.similarity_threshold:
                    ans = result
                    break
        except Exception:
            pass
    ...
    def status_checker(self, save_path: str = "papers") -> bool:
        downloaded = []
        ...
        def is_downloaded(title: str) -> bool:
            return (
                max(map(lambda x: self.similarity(title, x), downloaded))
                >= self.similarity_threshold
            )

        return is_downloaded

    def download(self, titles: List[str], save_path: str = "papers") -> int:
        checker = self.status_checker(save_path)
        count: int = 0
        pbar = track(titles)
        for title in pbar:
            if checker(title):
            ...
            if result is None:
                continue
            try:
                result.download_pdf("papers", f"{result.title.replace(' ', '_')}.pdf")
                count += 1
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    titles, _ = zip_longest(*papers, fillvalue="")
    total = downloader.download(titles)
    console.print(f"{total} papers downloaded")
